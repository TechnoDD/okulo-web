...
export const createPatient = createServerFn().handler(async ({ data }) => {
  return await tablesDB.createRow('69428862001e36e3a748', 'patients', 'unique()', { firstName: data.firstName, lastName: data.lastName, birthDate: new Date(data.birthDate).toISOString(), fiscalCode: data.fiscalCode })
})

export const updatePatient = createServerFn().handler(async ({ data }) => {
  return await tablesDB.updateRow('69428862001e36e3a748', 'patients', data.$id, { firstName: data.firstName, lastName: data.lastName, birthDate: new Date(data.birthDate).toISOString(), fiscalCode: data.fiscalCode })
})

export const deletePatient = createServerFn().handler(async ({ data }) => {
  return await tablesDB.deleteRow('69428862001e36e3a748', 'patients', data.userId)
})
