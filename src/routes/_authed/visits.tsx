...
function ImmaginiModale({ isOpen, onClose, visita }) {
    ...
    // Nomi dei 7 gruppi
    const nomiGruppi = [
        "Complete Hess chart comparison",
        "Hess chart comparison in left inferior rectus paresis",
        "Hess chart comparison in right inferior rectus paresis",
        "Hess chart comparison in left lateral rectus paresis",
        "Hess chart comparison in right lateral rectus paresis",
        "Hess chart comparison in left medial rectus paresis",
        "Hess chart comparison in right medial rectus paresis",
        "Hess chart comparison in left superior rectus paresis",
        "Hess chart comparison in right superior rectus paresis",
        "Hess chart comparison in left inferior oblique paresis",
        "Hess chart comparison in  right inferior oblique paresis",
        "Hess chart comparison in left superior oblique paresis",
        "Hess chart comparison in right superior oblique paresis",
    ];

    // Nomi specifici per le COPPIE
    const nomiCoppie = ["Complete", "Internal Random", "External Random"];
    ...
    async function generaPDF(anonymous = false) {
        ...
        const datiSinistra = [
            ["Name:", anonymous ? `${visita.patient || ""}`.trim().toUpperCase() : `${visita.patientData?.firstName || ""} ${visita.patientData?.lastName || ""}`.trim()],
            ["Sex:", visita.patientData?.gender || "N/D"],
            ["Age:", visita.patientData?.age || "N/D"],
            ["Appointment date:", visita.visitDate ? new Date(visita.visitDate).toLocaleDateString("en-EN") : "N/D"],
            ["Reason:", visita.reason || "N/D"],
        ];

        const datiDestra = [
            ["Pathologies:", visita.patientData?.pathologies || "None"],
            ["Notes:", visita.notes || "No notes"],
        ];
        ...
        const label1Width = helveticaFont.widthOfTextAtSize("Green in front of the L.E.", 11);
        const label2Width = helveticaFont.widthOfTextAtSize("Green in front of the R.E.", 11);
        page.drawText("Green in front of the L.E.", {
        ...
        page.drawText("Green in front of the R.E.", {
        ...
        // Footer
        const ora = new Date().toLocaleString("en-EN");
        const oraWidth = helveticaFont.widthOfTextAtSize(`Generated: ${ora}`, 10);
        page.drawText(`Generated: ${ora}`, {
                                                ...
                                                <div key={pulsante.id} className="group relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-4 hover:shadow-xl transition-all hover:-translate-y-1 cursor-pointer overflow-hidden border-2 border-gray-100 hover:border-purple-300 h-fit" onClick={() => handlePulsanteClick(pulsante)}>
                                                    ...
                                                    <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                                                        <div className="bg-white/95 backdrop-blur-sm px-6 py-3 rounded-2xl text-center shadow-2xl">
                                                            ...
                                                            <div>
                                                                <span className="text-lg font-bold text-gray-800 block">View</span>
                ...
                <div className="fixed inset-0 bg-black/95 backdrop-blur-sm z-[60] flex items-center justify-center p-4" onClick={closeImageViewer}>
                    <div className="max-w-full max-h-[90vh] w-full h-full flex flex-col bg-white/10 backdrop-blur-sm rounded-3xl" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-6 border-b border-white/30 rounded-t-3xl">
                            ...
                            <div className="flex items-center space-x-3">
                                <button onClick={() => generaPDF()} className="w-14 h-14 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 backdrop-blur-sm rounded-3xl flex items-center justify-center transition-all group shadow-xl" title="Download PDF">
                                    <svg className="w-7 h-7 text-white group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button onClick={() => generaPDF(true)} className="w-14 h-14 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 backdrop-blur-sm rounded-3xl flex items-center justify-center transition-all group shadow-xl" title="Download PDF">
                                    <svg className="w-7 h-7 text-white group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function GestioneVisite() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.patient || !formData.visitDate || !formData.reason) {
            alert("Patient name, date, and reason are required");
            return;
        }
        setLoading(true);
        setTimeout(async () => {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-12">
                    ...
                    {pazienteSelezionato && (
                        ...
                        >
                            {soloPazienteSelezionato ? "👁️ Show all Visits" : "✅ Only visits by this patient"}
                        </button>
                    )}
                </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                        <div className="flex items-stretch gap-6 mb-8">
                            <div className="flex flex-row items-center gap-3 flex-shrink-0">
                                <div className="w-12 h-12 bg-gradient-to-r from-[#48b671] to-[#48b671] rounded-2xl flex items-center justify-center">
                                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <h2 className="text-2xl font-bold text-gray-900 text-center">
                                    {isEditing ? "Modifica Visita" : "New Visit"}
                                </h2>
                            </div>
                            <div className="flex items-center gap-2 text-sm text-yellow-800 bg-yellow-100 px-4 py-2 rounded-xl border border-yellow-200 flex-1 min-w-0">
                                <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                </svg>
                                Remember to book ONLY ONE visit
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex gap-3">
                                <button type="submit" disabled={loading}
                                    ...
                                    {loading ? "Salvando..." : (isEditing ? "Aggiorna" : "Add")}
                                </button>
                                {isEditing && (
                    ...
                    <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                        ...
                        <div className="mb-6">
                            <div className="relative">
                                <input type="text" placeholder="Search by patient (first name or last name) or date (YYYY-MM-DD)..." value={searchTerm}
                        ...
                        {filteredVisite.length === 0 ? (
                            <div className="text-center py-16">
                                ...
                                <h3 className="text-lg font-semibold text-gray-900 mb-1">No visits</h3>
                                <p className="text-gray-500">Add your first visit or edit the filters</p>
                            </div>
                        ) : (
                                    ...
                                    <table className="w-full">
                                        ...
                                        <thead>
                                            <tr className="bg-gray-50">
                                                ...
                                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Reasons</th>
                                ...
                                {filteredVisite.length > visitsPerPage && (
                                    <div className="mt-6 flex items-center justify-between">
                                        <div className="text-sm text-gray-700">
                                            Viewed {indexOfFirstVisit + 1}-{Math.min(indexOfLastVisit, filteredVisite.length)} di {filteredVisite.length} visits
                                        </div>
